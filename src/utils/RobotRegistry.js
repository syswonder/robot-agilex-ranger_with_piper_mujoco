...
export class RobotRegistry {
  ...
  async _load() {
    ...
    if (new Set(index.robots).size !== index.robots.length) {
    ...
    if (!this.robots.has(index.defaultRobot)) {
      throw new Error(`Default robot is not registered: ${index.defaultRobot}`);
    }
    this.defaultRobot = index.defaultRobot;
    return this;
  }
