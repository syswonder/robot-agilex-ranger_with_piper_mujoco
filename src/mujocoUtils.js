...
export async function setupGUI(parentContext) {
  ...
  const loadSceneAndUpdate = async (scenePath, robotName) => {
    ...
    if (keyboardController.hasConfig(robot)) {
      await keyboardController.enable(robot, parentContext.model, parentContext.data, parentContext.mujoco);
    }
  };
...
/** Keep the initial scene usable on narrow screens without changing desktop framing. */
...
export async function loadSceneFromURL(mujoco, filename, parent) {
    ...
    for (let g = 0; g < model.ngeom; g++) {
      ...
      if (!currentMaterial) {
        ...
        if (texId != -1) {
          const repeatX = texId == 2 ? 50 : model.mat_texrepeat[(matId * 2) + 0];
          const repeatY = texId == 2 ? 50 : model.mat_texrepeat[(matId * 2) + 1];
