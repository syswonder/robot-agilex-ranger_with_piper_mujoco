...
class NativeSensorSuite:
    def __init__(self, model: mujoco.MjModel, data: mujoco.MjData, config: dict) -> None:
        ...
        self.data = data
        self.config = config
        self.geom_group = np.array([0, 0, 0, 1, 0, 1], dtype=np.uint8)
        self.lidar_config = config["lidar"]
        self.lidar_site = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_SITE, self.lidar_config["site"])
        ...
        self.gyroscope = mujoco.mj_name2id(
            model, mujoco.mjtObj.mjOBJ_SENSOR, self.imu_config["gyroscope"])
        self.accelerometer = mujoco.mj_name2id(
            model, mujoco.mjtObj.mjOBJ_SENSOR, self.imu_config["accelerometer"])
        self.cameras = {item["id"]: item for item in config.get("cameras", [])}
        self.camera_ids = {
            key: mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_CAMERA, item["camera"])
